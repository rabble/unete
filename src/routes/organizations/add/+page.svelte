<script lang="ts">
  ...
  import { initNostrLogin, checkExistingNostrLogin, ndk } from '$lib/stores/ndk';
  import { createOrganization } from '$lib/nostr/organizations';
  import { getTopics } from '$lib/topics';
  import { ensureConnection } from '$lib/stores/ndk';
  ...
  // Subscribe to NDK store to watch for signer changes
  $: isLoggedIn = Boolean($ndk?.signer);

  onMount(async () => {
    if (browser) {
      // Initial check for existing login
      isLoggedIn = await checkExistingNostrLogin();
      checkedLogin = true;  // Mark that we've completed the login check
    }
  });
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // Pass the NDK instance to createOrganization
...
<div class="max-w-4xl mx-auto px-4 py-12">
  ...
  {:else}
    <form 
      ...
      class="bg-white rounded-lg shadow-lg p-8 {!isLoggedIn ? 'opacity-50 pointer-events-none' : ''}"
    >
      <!-- Basic Information -->
